...
// jian.jone added begin

static void extract_envs_info(msgpack_object envs, char* cluster, char* project){
	int i;
...
*/
		...
		for(j = 0; j< m.via.map.size; j++) {
			...
			if (mysize == 2 && k.via.str.size == sizeof("name") -1 &&
                        !strncmp(k.via.str.ptr, "name", k.via.str.size) && 
                v.via.str.size == sizeof("project") -1 &&
                        !strncmp(v.via.str.ptr, "project", v.via.str.size)) {
                msgpack_object v1 = m.via.map.ptr[(j+1)%2].val;
                strncpy(project, v1.via.str.ptr, v1.via.str.size);
				printf("project=%s\n", project);
            }
...
static void extract_containers_envs(msgpack_object containers, char* cluster, char* project)
{
	int j;
	msgpack_object v;
	v = containers;
    	...
    	for (l = 0; l < k1.via.map.size; l++) {
        	...
        	v2 = k1.via.map.ptr[l].val;
           	if (k2.via.str.size == sizeof("env") -1 &&
           		...
           		extract_envs_info(v2, cluster, project);
				return; 
			}
        }
    }
                      ...
                      char **out_buf, size_t *out_size)
{
    ...
    if (spec_found == FLB_TRUE) {
        for (i = 0; i < spec_val.via.map.size; i++) {
            ...
            if (k.via.str.size == 8 &&
                ...
                have_nodename = i;
                map_size++;
                //break;//jian.jone mask this and add follow
    ...
    if (1) {
        ...
        msgpack_pack_str(&mp_pck, 7);
        msgpack_pack_str_body(&mp_pck, "project", 7);
        msgpack_pack_str(&mp_pck, len);
        msgpack_pack_str_body(&mp_pck, project, len);
    }
